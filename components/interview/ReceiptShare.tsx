...
export function ReceiptShare({
  ...
  const linkedInText = `I scored ${score}/100 on a verified ${skill} interview on Intervue. Proof, not promises 👇 ${receiptUrl}`
  const twitterText = `Scored ${score}/100 on a verified ${skill} interview 🎯\n\nAI-proctored, evidence-scored. Here's the receipt:\n\n${receiptUrl}`
  ...
  return (
    <div className="flex gap-2 w-full">
      <a
        href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(receiptUrl)}&summary=${encodeURIComponent(linkedInText)}`}
        target="_blank"
